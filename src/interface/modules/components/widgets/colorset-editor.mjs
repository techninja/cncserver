...
export default styles => ({
  ...
  render: ({
    items, activeName, activeTitle, activeColor, isEditing, errorName, errorTitle,
  }) => html`
    ${styles}
    ...
    name, title, color, active,
            ...
            <div title="Edit this Color" class="${{ 'colorset-item': true, card: true, active }}" onclick="${editItem(name)}">
              <header class="card-header">
                <b style="${{ backgroundColor: color }}">${name}</b>
                <p class="card-header-title">${title}</p>
                <a class="card-header-icon" onclick="${removeItem(title, name)}" title="Delete this color">
        ...
        <div class="columns is-multiline">
          <section class="column">
            ...
            <div class="field">
              ...
              <div class="control">
                <input class="input color" type="color" value="${activeColor}" onchange="${html.set('activeColor')}">
              </div>
          ...
          <section class="column is-half">
            ...
            <div class="field">
              <button-single onclick="${cancelEdit}" title="Cancel" icon="window-close" style="danger"></button-single>
              ${!isEditing && html`<button-single onclick="${saveItem('create')}" title="Add" icon="plus-circle" style="success"></button-single>`}
              ${isEditing && html`<button-single onclick="${saveItem('save')}" title="Save" icon="save" style="warning">${isEditing}</button-single>`}
            </div>
          </section>
        </div>
      </section>
