...
function moveRequest(req) {
  ...
  if (op === 'toward') {
    ...
    const point = {
      x: (parseInt(arg, 10) * sizeMultiplier) + cncserver.bot.workArea.absCenter.x,
      y: (-parseInt(arg2, 10) * sizeMultiplier) + cncserver.bot.workArea.absCenter.y,
    };
...
module.exports.initAPI = (cncserverArg) => {
  ...
  pollData.render = function renderData() {
    ...
    for (const [key, value] of Object.entries(cncserver.pen)) {
  ...
  cncserver.createServerEndpoint('/poll', () => (
  ...
  cncserver.createServerEndpoint('/crossdomain.xml', () => ({
    ...
    body:
      ...
      <allow-access-from domain="*" to-ports="${cncserver.gConf.get('httpPort')}
       "/></cross-domain-policy>`,
  }));
