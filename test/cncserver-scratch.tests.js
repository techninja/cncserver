...
describe('ScratchTests', function(){
  ...
  describe('Init start', function(){
    it('should be on by default', function(){
      expect(cncserver.said('Scratch v2 Programming support ENABLED')).to.equal(true);
    });

    it('[coord] should move to the center from park (3833, 1800)', function(done){
  ...
  describe('Basic Moving (forward, right, left, toward, absturn)', function(){
    it('[forward] should move 10 steps (100 steppers steps right)', function(done){
    ...
    it('[right] should turn right 90deg to 90deg (no movement)', function(done){
    ...
    it('[forward] should move 10 steps down (100 steppers steps)', function(done){
      req.get(path + 'move.forward./10').on('response', function(){
        setTimeout(function(){
    ...
    it('[right] should turn right 90deg to 180deg (no movement)', function(done){
      req.get(path + 'move.right./90').on('response', function(){
    ...
    it('[forward] should move 10 steps left (100 steppers steps)', function(done){
      req.get(path + 'move.forward./10').on('response', function(){
        setTimeout(function(){
          expect(cncserver.out[0]).to.contain('SM,67,-100,0');
          expect(cncserver.out[1]).to.equal('Move pen by 10 steps');
    ...
    it('[right] should turn right 90deg to 270deg (no movement)', function(done){
      req.get(path + 'move.right./90').on('response', function(){
    ...
    it('[toward] should turn towards (0,0) to 0deg (no movement)', function(done){
      req.get(path + 'move.toward./0/0').on('response', function(){
        expect(cncserver.out[0]).to.equal('Rotate relative towards 3832.5,1800 from 3733, 1800 to 0 deg');
        done();
      });
    });

    it('[absturn] should turn to 270deg (no movement)', function(done){
      req.get(path + 'move.absturn./270').on('response', function(){
        expect(cncserver.out[0]).to.equal('Rotate to 270 scratch degrees (actual angle 180deg)');
        done();
      });
    });

    it('[forward] should move -10 steps toward right (-100 steppers steps)', function(done){
      req.get(path + 'move.forward./-10').on('response', function(){
        setTimeout(function(){
          expect(cncserver.out[0]).to.contain('SM,67,100,0');
          expect(cncserver.out[1]).to.equal('Move pen by -10 steps');
  ...
  describe('Drawing outside boundary (forward, left, absturn, pen.down)', function(){
    it('[absturn] should turn to 0deg (no movement)', function(done){
      req.get(path + 'move.absturn./0').on('response', function(){
        expect(cncserver.out[0]).to.equal('Rotate to 0 scratch degrees (actual angle -90deg)');
        done();
      });
    });

    it('[pen.down] should put the pen down for drawing', function(done){
      req.get(path + 'pen.down').on('response', function(){
        setTimeout(function(){
          ...
          expect(cncserver.out[0]).to.contain('SM,67,0,0'); // Block until it's done moving
          done();
        }, 15); // Eat up the whole time taken for move
      });
    });

    it('[forward] should attempt to move 500 steps, lifting  ("5000" stepper steps, limited to 1800 steps)', function(done){
      req.get(path + 'move.forward./500').on('response', function(){
        setTimeout(function(){
          expect(cncserver.out[5]).to.equal('Move pen by 500 steps');
          expect(cncserver.out[4]).to.contain('SM,1200,0,-1800'); // Move position
          expect(cncserver.out[3]).to.equal('Skipping buffer to set height: 19750'); // Buffer skipped lift insertion
          expect(cncserver.out[2]).to.contain('SC,5,19750'); // Set Position
          expect(cncserver.out[1]).to.contain('SP,0'); // Go to position
          expect(cncserver.out[0]).to.contain('SM,137,0,0'); // Block till complete
    ...
    it('[left] should turn left 90deg to 180deg (left, no movement)', function(done){
      req.get(path + 'move.left./90').on('response', function(){
        expect(cncserver.out[0]).to.equal('Rotate left 90 deg. to 180 deg.');
        done();
      });
    ...
    it('[forward] should move 20 steps left (200 stepper steps)', function(done){
    ...
    it('[left] should turn left 90deg to 90deg (down, no movement)', function(done){
      req.get(path + 'move.left./90').on('response', function(){
        setTimeout(function(){
    ...
    it('[forward] should move 100 steps down (0 actual stepper steps)', function(done){
    ...
    it('[forward] should move 400 steps down (1800 actual stepper steps)', function(done){
      req.get(path + 'move.forward./400').on('response', function(){
        setTimeout(function(){
          expect(cncserver.out[5]).to.equal('Move pen by 400 steps');
          expect(cncserver.out[4]).to.equal('Go back to: draw'); // Move to "last" pen height
          expect(cncserver.out[3]).to.contain('SC,5,12750'); // Set Position
          expect(cncserver.out[2]).to.contain('SP,0'); // Go to position
          expect(cncserver.out[1]).to.contain('SM,137,0,0'); // Block till complete
          expect(cncserver.out[0]).to.contain('SM,1200,0,1800'); // Move position
          done();
